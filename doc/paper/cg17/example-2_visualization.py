...
def savefig(mesh, geom, data=None, label="", out=None, **kwargs):
    ...
    if data is not None:
        ax, _ = pg.show(mesh, data, hold=1, pad=0.55, label=label, ax=ax, **kwargs)
    if out:
...
def showSynthData(synthPath):
    ...
    # START Saturation
    axs = plt.subplots(
        row, col, sharex=True, sharey=True, figsize=(10.0 * 0.65, 7.25 * 0.65)
    )[1].flatten()
    ...
    for i, a in enumerate(axs):
        ...
        pg.viewer.mpl.drawSensors(a, scheme.sensorPositions(), diam=0.15, color="green")
...
def showModel(outPath):
    ...
    allModel[1] = fopModel[
        fopMesh.findCellByMarker(pg.MARKER_FIXEDVALUE_REGION - 1)[0].id()
    ]
