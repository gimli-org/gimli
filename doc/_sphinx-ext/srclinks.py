...
class Context(object):
    ...
    def __init__(
        ...
        (self.project_path, self.project_url) = self.get_project_vars_from_urlstr(
            self.project
        )
    ...
    def get_project_vars_from_urlstr(self, urlstr):
        ...
        if not _url.netloc:
            ...
            project_path = "/".join(
                [p for p in _path.split("/") if p][:2]
            )  # user/project
        if self.type_:
            project_url = "https://{type_}/{project_path}".format(
                type_=self.type_, project_path=project_path
            )
        else:
    ...
    def get_srclinks(self, project=None):
        ...
        if self.src_clonecmd_tmpl is not None:
            c["srclink_src_clone_cmd"] = self.src_clonecmd_tmpl.format(**conf)
        return c
...
class GitHubContext(Context):
    ...
    def is_github_url(urlstr=None, url=None):
        ...
        if url.netloc in ("git@github.com", "github.com") or url.path.startswith(
            "git@github.com/"
        ):
            return True
...
class BitbucketSourcelinkContext(Context):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (
            _url.path.startswith("hg@bitbucket.org/")
            ...
            _url.path.startswith("git@bitbucket.org/")
...
class BitbucketGitSourcelinkContext(BitbucketSourcelinkContext):
    ...
    def is_bitbucket_git_url(urlstr=None, url=None):
        if url is None:
            url = urlparse.urlparse(urlstr)
        if "git@bitbucket.org" == url.netloc or url.path.startswith(
            "git@bitbucket.org/"
        ):
            return True
...
def html_page_context(
    app=None, pagename=None, templatename=None, context=None, doctree=None, urlstr=None
):
    """
    ...
    def test_srclinks(self):
        ...
        def test_html_page_context(
            ...
            context = build_srclink_context(
                project=project, branch=project_branch, src_path=project_src_path
            )
            ...
            self.assertEqual(context["srclink_src_https_url"], project_https_url)
            self.assertEqual(context["srclink_src_ssh_url"], project_ssh_url)
            self.assertEqual(context["srclink_src_native_url"], project_native_url)
            return context
