...
u = solve(grid, f=1.0, bc={"Dirichlet": dirichletBC})
...
neumannBC = {1: -0.5, 4: 2.5}  # left  # bottom

dirichletBC = {3: 1.0}  # top

u = solve(grid, f=0.0, bc={"Dirichlet": dirichletBC, "Neumann": neumannBC})
...
ax.text(
    1.01, 0.0, "$\partial u/\partial n=0$", va="center", ha="left", rotation="vertical"
)  # right -- 2
...
u = solve(grid, f=1.0, bc={"Node": [grid.findNearestNode([0.0, 0.0]), 1.0]})
