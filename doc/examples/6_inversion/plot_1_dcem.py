...
errorEMabs = 1.0  # absolute (ppm of primary signal)
errorDCrel = 3.0  # in per cent
...
ab2 = 1.3 ** np.arange(20) * 3.0  # logarithmically equidistance starting with 3m
na = len(ab2)
fDC = VESModelling(ab2=ab2, mn2=np.ones_like(ab2))
dataDC = fDC(synThk + synRes)
errorDC = np.ones_like(dataDC) * errorDCrel / 100.0
dataDC *= 1.0 + pg.randn(len(dataDC), seed=1234) * errorDC
