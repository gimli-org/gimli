...
scheme = ert.createData(
    pg.utils.grange(start=1, end=24, dx=1, n=10, log=True), sounding=True
)
...
plc = mt.createWorld(start=[-200, -100], end=[200, 0], layers=[-10], area=[5.0, 500])
...
data = ert.simulate(mesh, res=[[1, 100.0], [2, 1.0]], scheme=scheme, verbose=False)
...
res = amps - 1j * amps * np.sin(phases / 1000.0)
data = ert.simulate(mesh, res=[[1, res[0]], [2, res[1]]], scheme=scheme, verbose=False)
...
ves.drawData(
    ax,
    pg.cat(data("rhoa"), -data("phia")),
    labels=[r"$\varrho_a$ 2D FEM", r"$\varphi_a$ 2D FEM"],
    marker="o",
    linestyle="none",
)
ves.drawData(ax, rc, labels=[r"$\varrho_a$ 1D VES", r"$\varphi_a$ 1D VES"], marker=None)
