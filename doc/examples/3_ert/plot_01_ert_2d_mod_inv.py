...
block = mt.createCircle(
    pos=[-5, -3.0], radius=[4, 1], marker=4, boundaryMarker=10, area=0.1
)

###############################################################################
poly = mt.createPolygon(
    [(1, -4), (2, -1.5), (4, -2), (5, -2), (8, -3), (5, -3.5), (3, -4.5)],
    isClosed=True,
    addNodes=3,
    interpolate="spline",
    marker=5,
...
scheme = ert.createData(elecs=np.linspace(start=-15, stop=15, num=21), schemeName="dd")
...
rhomap = [[1, 100.0], [2, 75.0], [3, 50.0], [4, 150.0], [5, 25]]
...
# testing purposes.
data = ert.simulate(
    mesh, scheme=scheme, res=rhomap, noiseLevel=1, noiseAbs=1e-6, seed=1337
)
...
pg.info(
    "Selected data noise %(min/max)", min(data["err"]) * 100, max(data["err"]) * 100
)
...
# point outside.
inversionDomain = pg.createGrid(
    x=np.linspace(start=-18, stop=18, num=33),
...
grid = pg.meshtools.appendTriangleBoundary(
    inversionDomain, marker=1, xbound=50, ybound=50
)
