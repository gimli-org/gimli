...
def plot(x, a1, ga, gza, a2, g, gz, legend=True):
    ...
    a1.plot(
        x,
        g[:, 0],
        label=r"Won & Bevis: $\frac{\partial u}{\partial x}$",
        marker="o",
        linewidth=0,
        c="red",
    )
    a1.plot(
        x,
        g[:, 2],
        label=r"Won & Bevis: $\frac{\partial u}{\partial z}$",
        marker="o",
        linewidth=0,
        c="blue",
    )
    ...
    a2.plot(
        x, gza[:, 0], label=r"Analytical $\frac{\partial^2 u}{\partial z,x}$", c="red"
    )
    a2.plot(
        x, gza[:, 1], label=r"Analytical $\frac{\partial^2 u}{\partial z,z}$", c="blue"
    )
...
# Horizontal cylinder
circ = createCircle([0, -depth], radius=radius, marker=2, area=0.1, nSegments=32)
...
thickness = 1
mesh = pg.createGrid(
    x=np.linspace(0, 5000), y=[-depth - thickness / 2.0, -depth + thickness / 2.0]
)
