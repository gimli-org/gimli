...
#    print("calcGBounds", swatch.duration(True))
    ...
    gAna = analyticalCircle2D(spnts, radius=2.0,
                              pos=pg.RVector3(0.0, -5.0), dDensity=2000)
    gAna2 = analyticalCircle2D(spnts, radius=2.0,
                               pos=pg.RVector3(5.0, -5.0), dDensity=2000)
...
#    rho=GB[0]/mesh.cellSizes()
    ...
    for i in (0, 1):
        drawSelectedMeshBoundaries(ax2, mesh.findBoundaryByMarker(i),
...
#    r0 = np.sqrt(v**2 + h**2)
    ...
    atanLambda = np.arctan2(2.0 * h * L, (r1 + r2 + L) * (r1 + r2 - L) +
                            2.*abs(v) * (r2 + r1))  # (39)
...
def test3d():
    ...
    gAna = analyticalSphere(spnts, radius=2.0, pos=pg.RVector3(0.0, 0.0, -5.0),
                            dDensity=2000)
