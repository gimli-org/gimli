...
//                      nodeVector_[ 0 ]->pos()[ 1 ] + y21 * rst[ 0 ] + y31 * rst[ 1 ] );
    ...
    return nodeVector_[ 0 ]->pos() 
            + ( nodeVector_[ 1 ]->pos() - nodeVector_[ 0 ]->pos() ) * rst[ 0 ] 
            + ( nodeVector_[ 2 ]->pos() - nodeVector_[ 0 ]->pos() ) * rst[ 1 ] ;
}
...
RVector3 QuadrangleShape::xyz( const RVector3 & rst ) const {
//** Coordinate transformation
// **     xp = x1 + (x2 - x1 ) * r + (x4 - x1 ) * s - [(x2 - x1 ) + (x4 - x3 )] * r * s
// **     yp = y1 + (y2 - y1 ) * r + (y4 - y1 ) * s - [(y2 - y1 ) + (y4 - y3 )] * r * s
    double x21 = nodeVector_[ 1 ]->pos()[ 0 ] - nodeVector_[ 0 ]->pos()[ 0 ];
    ...
    return RVector3( nodeVector_[ 0 ]->pos()[ 0 ] + x21 * rst[ 0 ] + x41 * rst[ 1 ] - ( x21 + x43) * rst[ 0 ] * rst[ 1 ],
                     nodeVector_[ 0 ]->pos()[ 1 ] + y21 * rst[ 0 ] + y41 * rst[ 1 ] - ( y21 + y43) * rst[ 0 ] * rst[ 1 ] );

    return nodeVector_[ 0 ]->pos()
            + ( nodeVector_[ 1 ]->pos() - nodeVector_[ 0 ]->pos() ) * rst[ 0 ]
            + ( nodeVector_[ 3 ]->pos() - nodeVector_[ 0 ]->pos() ) * rst[ 1 ]
            - ( ( nodeVector_[ 1 ]->pos() - nodeVector_[ 0 ]->pos() ) +
                ( nodeVector_[ 3 ]->pos() - nodeVector_[ 2 ]->pos() ) ) * rst[ 0 ]*rst[ 1 ] ;
