...
def testShowVariants():
    ...
    pg.show(
        mesh, mesh.cellMarkers(), label="Cell markers", showBoundary=False, ax=axs[1][4]
    )
...
def testColorbar():
    ...
    grid = pg.createGrid(x=np.linspace(10.0, 110.0, 11) - 5, y=np.linspace(0.0, 20, 2))

    fig, axs = plt.subplots(nrows=3, ncols=3, figsize=(10, 6))
    ax, cbar = pg.show(
        grid,
        data=pg.x(grid.cellCenter()),
        label="log x",
        ax=axs[0][0],
        showMesh=True,
        cMap="Paired",
    ...
    ###########################################################################
    grid = pg.createGrid(x=np.linspace(10.0, 110.0, 11) - 25, y=np.linspace(0.0, 20, 2))
    ax, cbar = pg.show(
        grid,
        data=pg.x(grid.cellCenter()),
...
def testColRange():
    ...
    data = np.arange(mesh.cellCount())
    pg.show(
        mesh, data, ax=ax[0][1], shading="gouraud", label="default (nLevs=5, nCols=256)"
    )
    pg.show(
        mesh,
        data,
...
def testShowPV():
    ...
    pg.viewer.pv.drawMesh(
        ax,
        m2,
        label="m",
        filter={"threshold": dict(value=14, scalars="m", invert=True)},
    )
    pg.viewer.pv.drawMesh(
        ax, m2, label="m", filter={"slice": dict(origin=[1, 1, 1], normal=[1, 1, 0])}
    )
    ax.show()
    ...
    pg.viewer.pv.drawStreamLines(
        ax, m1, vel, radius=0.01, source_radius=0.5, source_center=[-0.5, -0.0, -0.0]
    )
