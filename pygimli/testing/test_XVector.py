...
class TestRVectorMethods(unittest.TestCase):
    ...
    def test_Slices(self):
        ...
        a = pg.Vector(np.arange(10.0))

        np.testing.assert_array_equal(a[:], np.arange(10.0)[:])
        np.testing.assert_array_equal(a[:, np.newaxis], np.array(a)[:, np.newaxis])
        np.testing.assert_array_equal(a[::], np.arange(10.0)[::])
        np.testing.assert_array_equal(a[:-1], np.arange(10.0)[:-1])
        np.testing.assert_array_equal(a[:-9], np.arange(10.0)[:-9])
        np.testing.assert_array_equal(a[:-10], np.arange(10.0)[:-10])

        np.testing.assert_array_equal(a[::1], np.arange(10.0)[::1])
        np.testing.assert_array_equal(a[::-1], np.arange(10.0)[::-1])

        np.testing.assert_array_equal(a[0:3:1], np.arange(10.0)[0:3:1])
        np.testing.assert_array_equal(a[0:3:2], np.arange(10.0)[0:3:2])

        np.testing.assert_array_equal(a[3:0:-1], np.arange(10.0)[3:0:-1])
        np.testing.assert_array_equal(a[3:0:-2], np.arange(10.0)[3:0:-2])
