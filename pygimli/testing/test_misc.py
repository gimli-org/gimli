...
class TestMisc(unittest.TestCase):
    ...
    def test_Trans(self):
        ...
        f = pg.trans.TransLin(factor=2.0, offset=4.0)
        np.testing.assert_array_equal(f(x), x * 2.0 + 4.0)
        np.testing.assert_array_equal(f.trans(x), x * 2.0 + 4.0)
        np.testing.assert_array_equal(f.inv(f(x)), x)
        ...
        np.testing.assert_array_equal(f.update([1.0], [100.0]), [10.0])
        ...
        np.testing.assert_array_equal(
            f.at(0).fwd(np.ones(10) * 10), np.log(np.ones(10) * 10)
        )
        np.testing.assert_array_equal(f.fwd(np.ones(10) * 10), np.log(np.ones(10) * 10))
    ...
    def test_DataContainerSensors(self):
        ...
        self.assertEqual(data.sensor(0), [4.0, 0.0, 0.0])
        self.assertEqual(data.sensor(4), [0.0, 0.0, 0.0])
    ...
    def test_DataContainerIndex(self):
        ...
        data["a"][
            0
        ] = 1.0  # will not work for sensorIndex until its changed in the datacontainer as IndexArray
    ...
    def test_Hash(self):
        ...
        self.assertFalse(pg.Vector(1, 0.0).hash() == pg.Vector(2, 0.0).hash())
        ...
        self.assertEqual(v1.hash(), pg.Vector(10, 2.0).hash())
