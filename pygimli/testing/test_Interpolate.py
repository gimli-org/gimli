...
class TestInterpolate(unittest.TestCase):
    ...
    def test_Interpolate(self):
        ...
        u = pg.Vector(grid.nodeCount(), 1.0)

        # test with pg.interpolate
        queryPos = [0.2, 0.2]
        uI = pg.interpolate(srcMesh=grid, inVec=u, destPos=[queryPos, queryPos])

        np.testing.assert_allclose(uI[0], 1.0)

        # test manual interpolation
        c = grid.findCell(queryPos)
        uI = c.pot(queryPos, u)
        np.testing.assert_allclose(uI, 1.0)
        ...
        I = grid.interpolationMatrix([[0.0, 0.0], [1.0, 0.0], [1.0, 1.0], [0.0, 1.0]])
        uI = I * u
        ...
        np.testing.assert_allclose(
            pg.interpolate(grid, pg.x(grid.positions()), x, x * 0.0), x
        )
        np.testing.assert_allclose(
        ...
        np.testing.assert_allclose(
            pg.interpolate(grid, pg.x(grid.positions()), x=x, y=x * 0, z=x * 0), x
        )
        x = pg.Vector(x)
        ...
        np.testing.assert_allclose(
            pg.interpolate(grid, pg.x(grid.positions()), pnts), x
        )
        np.testing.assert_allclose(
            pg.interpolate(grid, pg.x(grid.positions()), np.array(pnts)), x
        )
