...
class TestFiniteElementBasics(unittest.TestCase):
    ...
    def test_Helmholtz(self):
        ...
        uFEM = pg.solve(mesh, a=1, b=k, f=f(x, k), bc={"Dirichlet": dirichletBC})
    ...
    def test_Neumann_BC(self):
        def _test_(mesh, p2=False, show=False):
            ...
            if show:
                if mesh.dim() == 1:
                    ...
                    model, response = pg.frameworks.fit(
                        uExact, u[idx], x=pg.x(mesh)[idx]
                    )
                    print(model)
            ...
            np.testing.assert_allclose(pg.solver.intDomain(u, mesh), 0.0, atol=1e-8)
        ...
        # 2D tri
        mesh = pg.meshtools.createMesh(
            pg.meshtools.createWorld(start=[0, 0], end=[1, 1], worldMarker=False),
            area=0.05,
        )
        ...
        mesh = pg.meshtools.createMesh(
            pg.meshtools.createWorld(start=[0, 0, 0], end=[1, 1, 1], worldMarker=False),
            area=0.1,
        )
    ...
    def test_Dirichlet_BC(self):
        ...
        def _testP1_(mesh, show=False, followP2=True):
            ...
            u = pg.solve(mesh, a=1, b=0, f=0, bc={"Dirichlet": {1: 0, 2: 1}})
        ...
        # 2D reg quad
        _testP1_(
            pg.createGrid(x=np.linspace(-2, 2, 11), y=np.linspace(0, 1, 11)), show=False
        )
        # 2D reg quad scale
        _testP1_(
            pg.createGrid(x=np.linspace(-0.04, 0.04, 11), y=np.linspace(-0.4, 0, 11))
        )
        # 3D reg quad
        _testP1_(
            pg.createGrid(
        ...
        # 2D tri
        mesh = pg.meshtools.createMesh(
            pg.meshtools.createWorld(start=[-1, -4], end=[1, -6], worldMarker=0),
            area=0.1,
        )
        ...
        # 3D tet
        mesh = pg.meshtools.createMesh(
            pg.meshtools.createCube(size=[4, 4, 4], boundaryMarker=9, area=1.1)
        )
    ...
    def test_Robin_BC(self):
        ...
        u = pg.solve(
            mesh, bc={"Node": [n, uExact(mesh.node(n).x())], "Neumann": {1: -A, 2: A}}
        )
        np.testing.assert_allclose(u, uExact(x))
