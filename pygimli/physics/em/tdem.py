...
def rhoafromU(U, t, Tx, current=1.0, Rx=None):
    ...
    mu0 = 4e-7 * pi
    rhoa = (Rx * Tx * mu0 / 20.0 / UbyI) ** (2.0 / 3.0) * t ** (-5.0 / 3.0) * mu0 / pi
    return rhoa
...
def rhoafromB(B, t, Tx, current=1):
    ...
    """
    mu0 = 4e-7 * pi
    rhoa = (current * Tx * mu0 / 30.0 / B) ** (2.0 / 3.0) * mu0 / pi / t
    return rhoa
...
def RxArea(snd):
    ...
    if "COIL_SIZE" in snd:
        Rx = snd["COIL_SIZE"]
        if Rx == 700.0:
            Rx = 100.0  # hack for wrong turns in NMR noise loop
            ...
            if zeile[0] == "/":  # comment-like
                if zeile[1:4] == "END":  # end of a sounding
                    if isdata:  # already read some data
                        ...
                        if not (
                            stripnoise
                            and "SWEEP_IS_NOISE" in sounding
                            and sounding["SWEEP_IS_NOISE"] == 1
                        ):
...
def readTEMfastFile(temfile):
    ...
    t, v, e, r = np.loadtxt(temfile, skiprows=8, usecols=(1, 2, 3, 4), unpack=True)
...
def readSiroTEMData(fname):
    ...
    while line[0] != ";":
        ...
        while len(line) > 0:
            ...
            nums = [
                float(el[-7:-2]) * 10 ** (float(el[-2:]))
                for el in line[1:-5].split(",")[1:]
            ]
...
class TDEM:
    ...
    def gather(self, token):
        ...
        if token == "LOOP_SIZE":
            mylist = [
                np.array(snd["LOOP_SIZE"].split(), dtype=float) for snd in self.DATA
            ]
        else:
    ...
    def plotTransients(self, ax=None, **kwargs):
        ...
        for i, data in enumerate(self.DATA):
            ...
            col = cols[i % len(cols)]
            pl.append(ax.loglog(t, u, label=getname(data), color=col, **kwargs))
            if "ST_DEV" in data:
    ...
    def plotRhoa(self, ax=None, ploterror=False, corrramp=False, **kwargs):
        ...
        for i, data in enumerate(self.DATA):
            ...
            err[err > 0.99] = 0.99
            col = "C" + str(i % 10)
            ax.loglog(rhoa, t, label=getname(data), color=col, **kwargs)  # color=col,
            if ploterror:
    ...
    def invert(self, nr=0, nlay=4, thickness=None, errorFloor=0.05):
        """Do inversion."""
        snd = self.DATA[nr]
        self.fop = VMDTimeDomainModelling(snd["TIME"], TxArea(snd), 1, nLayers=nlay)
        ...
        self.model = self.INV.run(dataVals=rhoa, errorVals=errorVals, startModel=model)
        return self.model
