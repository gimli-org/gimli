...
class HEMmodelling(Block1DModelling):
    ...
    def calc_forward(self, x, h, rho, d, epr, mur, quasistatic=False):
        ...
        if d.size:
            ...
            for n in range(self.f.size):
                for m in range(x.size):
                    field[n, m] = self.vmd_hem(
                        np.array([h[n, m]], float),
    ...
    def downward(self, rho, d, z, epr, mur, lam):
        ...
        alpha = np.sqrt(
            lam**2
            - np.tile(self.wem, (nl, lam.shape[1], 1)) * epr * mur
            + np.tile(self.iwm, (nl, lam.shape[1], 1)) * mur / rho
        )
            ...
            if (ind + 1) < nl:
                a = (
                    ...
                    * (
                        ...
                        - (b[ind + 1, :, :] - alpha[ind, :, :])
                        / (b[ind + 1, :, :] + alpha[ind, :, :])
                        * np.exp(-alpha[ind, :, :] * (d[ind, :, :] + h[ind + 1] - z))
                    )
                )
                ap = (
                    np.prod(aap[:ind, :, :], 0)
    ...
    def vmd_hem(self, h, rho, d, epr=1.0, mur=1.0, quasistatic=False):
        ...
        if np.any(index):
            opt = np.floor(
                10.0 * np.log10(self.r[index] * 2.0 * np.pi * self.f[index] / self.c0)
                + nc0
            )
            r0[index] = (
                self.c0
                / (2.0 * np.pi * self.f[index])
        ...
        q = 0.1 * np.log(10)
        lam = np.reshape(
            np.exp(-n[np.newaxis, :, np.newaxis] * q) / r0[np.newaxis, np.newaxis, :],
            (-1, nc, r0.size),
        )
        # (1, 100, nfreq)
        ...
        aux0 = (
            np.sum(
                ...
                * lam**3
                / alpha0
                * np.tile(fc0[::-1].T[:, :, np.newaxis], (1, 1, self.f.size)),
                1,
                complex,
            )
            / r0
        ...
        if np.any(index):
            ...
            aux3 = (
                np.sum(
                    delta3
                    * lam**2
                    * np.tile(fc1[::-1].T[:, :, np.newaxis], (1, 1, self.f.size)),
                    1,
                    complex,
                )
                / r0
        ...
        if np.any(index):
            Z[:, index] = (
                -self.r[index] ** 3 * aux1[:, index]
                + self.r[index] ** 3 * aux2[:, index]
                - self.r[index] ** 4 * aux3[:, index]
            ) * self.scaling
    ...
    def vmd_total_Ef(self, h, z, rho, d, epr, mur, tm):
        ...
        alpha0 = np.sqrt(
            lam**2 - np.tile(self.wem, (1, nc, 1)) + np.tile(self.iwm, (1, nc, 1)) / 1e9
        )
...
class HEMRhoModelling(HEMmodelling):
    ...
    def __init__(self, dvec, height=1.0, **kwargs):
