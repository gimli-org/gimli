...
def createData(elecs, schemeName="none", **kwargs):
    ...
    **kwargs :
    ...
    if isinstance(elecs, int):
        data = scheme.create(
            nElectrodes=elecs, electrodeSpacing=kwargs.pop("spacing", 1), **kwargs
        )
...
class DataSchemeBase(object):
    ...
    def create(self, nElectrodes=24, electrodeSpacing=1, sensorList=None, **kwargs):
        """."""
    ...
    def createElectrodes(self, nElectrodes=24, electrodeSpacing=1, sensorList=None):
        self.data_ = pg.DataContainerERT()

        if sensorList is not None:
            for p in sensorList:
                if isinstance(p, float):
                    self.data_.createSensor((p, 0.0))
                else:
                    self.data_.createSensor(p)
        else:
            for i in range(nElectrodes):
                self.data_.createSensor(pg.Pos(float(i) * electrodeSpacing, 0.0))
    ...
    def createDatum_(self, a, b, m, n, count):
        if (
            a < self.nElectrodes_
            and b < self.nElectrodes_
            and m < self.nElectrodes_
            and n < self.nElectrodes_
        ):
            if self.inverse_:
