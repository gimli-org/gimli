...
def combineMultipleData(DATA):
    ...
    data = DATA[0]
    if not data.haveData('k'):  # just do that only once
        data['k'] = createGeometricFactors(data)  # check numerical
    # should be generated by unique of all uniqueERTIndex
    R = np.ones([data.size(), len(DATA)]) * np.nan
    ERR = np.zeros_like(R)
    ...
    RHOA = np.reshape(data['k'], [-1, 1]) * R
    return data, RHOA, ERR
