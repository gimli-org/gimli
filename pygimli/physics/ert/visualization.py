    ...
    if "x" in kwargs and "y" in kwargs:
        ...
        if not equidistant:
            try:
                ...
                pg.warning(
                    "Something gone wrong while drawing data. "
...
def drawERTData(ax, data, vals=None, **kwargs):
    ...
    **kwargs:
    ...
    if ind is not None:
        ...
        mid, sep = midconfERT(data, circular=kwargs.get("circular", False), switch=sw)

    # var = kwargs.pop('var', 0)  # not used anymore
    cbar = None

    dx = kwargs.pop("dx", np.median(np.diff(np.unique(mid)))) * 2
    ax, cbar, ymap = showValMapPatches(vals, xVec=mid, yVec=sep, dx=dx, ax=ax, **kwargs)
...
def midconfERT(data, ind=None, rnum=1, circular=False, switch=False):
    ...
    if len(ux) * 2 > data.sensorCount() and not circular:  # 2D with topography case
        ...
        dxM = pg.mean(dx)
        if min(pg.y(data)) != max(pg.y(data)) or min(pg.z(data)) != max(pg.z(data)):
            # Topography case
        ...
        if max(dx) < 0.5:
            ...
            rnum = 3
        xe = np.hstack((0.0, np.cumsum(np.round(dx, rnum)), np.nan))
    ...
    sep[ialfa] = (
        spac * mI + (mn[ialfa] - 1) * mO * (abmn3 != 0) + 3 * mT + (abmn3 < 0) * mT
    )
    # gradient
