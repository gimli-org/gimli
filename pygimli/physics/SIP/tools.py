...
def fitCCCC(
    ...
    tLog = pg.trans.TransLog()
    fCC.region(0).setStartValue(1.0 / max(amp))
    if mpar[0] == 0:
        mpar[0] = 1.0 - min(amp) / max(amp)
    fCC.region(1).setParameters(*mpar)  # m (start,lower,upper)
    fCC.region(2).setParameters(*taupar)  # tau
    fCC.region(3).setParameters(*cpar)  # c
    data = pg.cat(1.0 / amp * np.cos(phi), 1.0 / amp * np.sin(phi))
