...
def drawAmplitudeSpectrum(
    ax,
    freq,
    amp,
    ylabel=r"$\rho$ ($\Omega$m)",
    grid=True,
    marker="+",
    ylog=True,
    **kwargs
...
def drawPhaseSpectrum(
    ax, freq, phi, ylabel=r"$-\phi$ (mrad)", grid=True, marker="+", ylog=False, **kwargs
):
...
def plotSpectrum(
    ax, freq, vals, ylabel=r"$-\phi$ (mrad)", grid=True, marker="+", ylog=True, **kwargs
):
