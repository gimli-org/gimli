...
def draw1DColumn(
    ...
    **kwargs
    ...
    z = -np.hstack([0.0, np.cumsum(thk), np.sum(thk) * 1.5]) + ztopo
    recs = []
    for i in range(len(val)):
        recs.append(Rectangle((x - width / 2.0, z[i]), width, z[i + 1] - z[i]))
...
def draw1dmodelErr(x, xL, xU=None, thk=None, xcol="g", ycol="r", **kwargs):
    ...
    plt.ylim(zm[-1] * 1.1, 0.0)
    plt.errorbar(
        x, zm, fmt=".", xerr=np.vstack((x - xL, xU - x)), ecolor=xcol, **kwargs
    )
    plt.errorbar(
        (x[:-1] + x[1:]) / 2,
        zc,
...
def showStitchedModels(
    ...
    **kwargs
    ...
    dxmed2 = kwargs.pop("width", np.median(np.diff(x))) / 2.0
    patches = []
    zMinLimit = 9e99
    zMaxLimit = 0
    ...
    if thk is not None:
        ...
        nlay = int(np.floor((len(models[0]) + 1) / 2.0))
    ...
    for i, imod in enumerate(models):
        ...
        if zMax > 0:
            z = np.hstack((0.0, np.cumsum(thki), zMax))
        else:
            thki = np.hstack((thki, thki[-1] * 3))
            z = np.hstack((0.0, np.cumsum(thki)))
