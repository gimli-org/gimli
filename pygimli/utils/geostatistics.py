...
def covarianceMatrixPos(pos, **kwargs):
    ...
    return covarianceMatrixVec(
        np.array(pg.x(pos)), np.array(pg.y(pos)), np.array(pg.z(pos)), **kwargs
    )
...
def generateGeostatisticalModel(mesh, nodes=False, seed=None, **kwargs):
    ...
    **kwargs
    ...
    return rng.multivariate_normal(
        np.ones(mesh.cellCount()), covarianceMatrix(mesh, nodes=nodes, **kwargs)
    )
...
def computeInverseRootMatrix(CM, thrsh=0.001, verbose=False):
    ...
    if verbose:
        print(
            "(C) Calculation time for eigenvalue decomposition:\n%s sec"
            % (time.time() - t)
        )
