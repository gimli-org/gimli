...
def interperc( a, trimval=3.0, islog=False, bins=None ):
    return inthist( a, np.array( [ trimval, 100. - trimval ] ), bins=bins, islog=islog )
...
#    plt.cla()
    ...
    if xlab is not None: plt.xlabel(xlab,fontsize=fs)
    if zlab is not None: plt.ylabel(zlab,fontsize=fs)
    plt.grid(which='both')
    #plt.show()
    return li
...
def draw1dmodelErr( x, xL, xU=None, thk=None, xcol='g', ycol='r', **kwargs ):
    ...
    plt.ylim( zm[-1]*1.1, 0. )
    plt.errorbar( x, zm, fmt='.', xerr=np.vstack( ( x - xL, xU - x ) ), ecolor=xcol, **kwargs )
    plt.errorbar( ( x[:-1]+x[1:] ) / 2, zc, fmt='.', yerr=np.vstack( ( thk-thkL, thkU-thk ) ), ecolor=ycol )
