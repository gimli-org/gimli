...
def assembleNeumannBC(S,
    ...
    boundaryPair    : tuple
        ...
        Value can be a scalar (float or int) or a function that is called
...
def assembleDirichletBC(S, boundaryPair, rhs=None, time=0.0,
    ...
    if type(boundaryPair) == tuple or len(boundaryPair) == 2:
        ...
        raise Exception("cannot interpret boundaries sequence:" +
                        str(type(boundaryPair)))  # uDirchlet does not exist!
...
def createMassMatrix(mesh, b=None):
    """
    Calculates mass element matrix for given mesh scaled by cell values b.
...
def solveFEM(mesh, a=1.0, b=0.0, f=0.0, times=None, userData=None,
    ...
    Variables are displayed in typewriter font, by using \\mathtt{var}:
    ...
    if times is None:
        ...
        for n in range(1, len(times)):
            ...
            swatch.reset()
            # (A + a*B)u is fastest, followed by A*u + (B*u)*a
            # and finally A*u + a*B*u and
