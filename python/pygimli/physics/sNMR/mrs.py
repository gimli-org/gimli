...
# def
# __init__(self,name=None,defaultNoise=150e-9,usereal=False,verbose=True):
# # constructor with dummy values
    ...
    def loadMRSI(
            self, filename, defaultNoise=100e-9, usereal=False, mint=0., maxt=2.0):
    ...
    def showResultAndFit(self, figsize=(12, 10), save='', plotmisfit=False,
        ...
        if plotmisfit:
            ...
            self.showCube(
                ax[1, 2], (self.data - self.INV.response()) / self.error, islog=False)
    ...
    def runEA(self, nlay=None, type='GA', pop_size=100,
        ...
        def datafit(individual, args):
            misfit = (
                self.data - self.f.response(self.genMod(individual))) / self.error
