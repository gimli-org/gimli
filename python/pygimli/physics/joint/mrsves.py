...
class MRSVES( MRS ):
    ...
    def createFOP(self, nlay, verbose=False):
        ...
        self.nlay = nlay
        self.f = MRSVESBlockModelling(nlay,self.K,self.z,self.t,self.ab2,self.mn2,verbose)
...
#        self.INV.setMaxIter(1)
    ...
    def plotResultAndFit(self, filename=None, figsize=(10,10)):
        ...
        self.showCube(ax[1,1],resp[:len(self.data)]*1e9,islog=False,clim=clim)
...
if __name__ == "__main__":
    ...
    parser = OptionParser("usage: %prog [options] basename", version="%prog: " + pg.__version__ )
    parser.add_option("-v", "--verbose", dest="verbose", action="store_true"
                            , help="be verbose", default=False)
    parser.add_option("-n", "--nLayers", dest="nlay",
                            help="number of layers", type = "int", default = "5")
    parser.add_option("-G", "--nsga2", dest="ga", action="store_true"
                            , help="use NSGA-II algorithm", default=False)
    parser.add_option("-U", "--uncertainty", dest="uncertainty", action="store_true"
                            , help="compute uncertainty", default=False)
