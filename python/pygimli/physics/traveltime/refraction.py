...
class BaseMethodeManager(object):
    ...
    auffüllen das im Grunde nur noch die Transen und der FOP erzwungen werden muessen.
    Die setter sollten die einzigen sein die automatisch mit inv und fop interagieren .. 
    also load, create etc. sollten immer mit set enden. DRY. 
    
    Es sollte immer ein self.model() geben das dann eben die lokalen velocity, resist. etc. zurück gibt.
    '''
    def __init__(self, **kwargs):
        
        # defaults that are allway usefulle
    ...
    def createFop(self, verbose=True): #need allway to be overloaded
        """
        """
        fop = None
        throw('abstract need to be overloaded') # always
        return fop
    ...
    def createInv(self, fop, verbose=True, dosave=False): # guter default möglich
        """
        """
    ...
    def setData(self, data): # guter default möglich
        """
        """
    ...
    def run(self, **kwargs): # guter default möglich
        """
        """
        throw('abstract need to be overloaded') 
...
class Refraction(object):
    ...
    def checkData(self):
        ...
        if self.verbose: print(self.dataContainer)
    ...
    def showData(self, ax=None, response=None):
        ...
        if response is None:
            ...
            plotFirstPicks(ax, self.dataContainer, np.asarray(response), marker='-')
    ...
    def run(self, vtop=500., vbottom=5000., zweight=0.2, lam=30.):
        ...
        """
        self.start = createGradientModel2D(self.dataContainer, self.mesh, vtop, vbottom)
    ...
    def rayCoverage(self):
        """return ray coverage"""
        return self.fopjacobian().transMult(pg.RVector(self.dataContainer.size(), 1.))
    ...
    def showResult(self, ax=None, cMin=None, cMax=None, logScale=False,
        ...
        # this will block all further shows until model view is closed
        #plt.show()  # block=False)
