...
class MPLRubberBander:
    ...
    def registerEventHandlers(self):
        ...
        if self.mouseMoveID_ is None:
            self.mouseMoveID_ = self.canvas_.mpl_connect(
                'motion_notify_event',
                self.onMouseMove)
        if self.mousePressID_ is None:
            self.mousePressID_ = self.canvas_.mpl_connect(
                'button_press_event',
                self.onMousePress)
        if self.mouseReleaseID_ is None:
            self.mouseReleaseID_ = self.canvas_.mpl_connect(
    ...
    def releaseEventHandlers(self):
        ...
        if self.mousePressID_ is not None:
            self.mousePressID_ = self.canvas_.mpl_disconnect(
                self.mousePressID_)
        if self.mouseReleaseID_ is not None:
            self.mouseReleaseID_ = self.canvas_.mpl_disconnect(
                self.mouseReleaseID_)
    ...
    def convertCoords(self, x, y):
        ...
        # print x, y, self.canvas_._lastx, self.canvas_._lasty,
        # self.canvas_.GetSize()

        pos = (min(max(0, x), self.canvas_.GetSize()[0]), min(
            max(0, self.canvas_.GetSize()[1] - y), self.canvas_.GetSize()[1]))
        # print pos
        return pos
    ...
    def onMouseRelease(self, event):
        ...
        if abs(selection[0] - selection[2]
               ) < 5 or abs(selection[1] - selection[3]) < 5:
            self.button_ = 0  # end of clicking
            return

        if self.lastAxes_ is not None:
            leX, leY = self.lastAxes_.transData.inverted().transform_point(
                self.convertCoords(
                    selection[0], selection[1]))
            riX, riY = self.lastAxes_.transData.inverted().transform_point(
                self.convertCoords(
            ...
            if self.button_ == 1 or self.button_ == 3:
                ...
                if self.zoomCallback_ is not None:
                    self.zoomCallback_(
                        (selection, (leX, leY, riX, riY), self.lastAxes_))
        else:
    ...
    def onMouseMove(self, event):
        if event is not None:
            ...
            if self.button_ == 1:
                ...
                x0, y0 = self.lastAxes_.transData.inverted().transform_point(
                    (0.0, 0.0))
                x1, y1 = self.lastAxes_.transData.inverted().transform_point(
                    self.endPoint_ - self.startPoint_)
...
#                                    )
        ...
        dec = self.AddSimpleTool(
            self.CUSTOM_DEC_CANVAS_WIDTH_ID,
            'Decrease canvas',
        ...
        self.canvasComboBox = ListCtrlComboPopupControl(self, style=wx.CB_READONLY | wx.CB_DROPDOWN  # |wx.NO_BORDER
                                                        , size=(87, -1)
                                                        )
        ...
        inc = self.AddSimpleTool(
            self.CUSTOM_INC_CANVAS_WIDTH_ID,
            'Increase canvas',
        ...
        self.AddSimpleTool(
            self.CUSTOM_FIT_CANVAS_ID,
            'Fit canvas',
            loadIcon("zoom-fit-best.png"),
            'Fit canvas to image size')

        self.aspectTool = self.AddSimpleTool(
            self.CUSTOM_ASPECT_AUTO_ID,
            'Auto aspect',
    ...
    def onZoomButton(self, event):
        ...
        ''
        if event.Checked():
            self.rubberBander_.start(
                zoomCallback=self.zoom,
                panCallback=self.pan)
        else:
    ...
    def onIncCanvasWidth(self, event):
        ...
        if curSelection < len(self.canvasWidthScales) - 1:
            if self.canvasWidthScales[
                    curSelection + 1] > self.canvasWidthScales[curSelection]:
 ...
 #       print "onCanvasWidthCombo: ", event, curSelection
    ...
    def setAspectAuto(self, val=True):
        ' Set auto aspect '
        if val:
            # if self.aspectTool.GetState() is not
...
class wxMatplotPanel(scrolled.ScrolledPanel):
    ...
    def refresh(self):
        ...
        pixels = tuple(
            [int(self.GetSize()[0] * self.canvasZoomWidth), int(self.GetSize()[1] * 1.0)])
