...
class PatchedAuiToolBar(TOOLBAR):
    ...
    def Realize(self):
        ...
        if gripper_size > 0 and self._gripper_visible:
            if horizontal:
                self._gripper_sizer_item = sizer.Add(
                    (gripper_size, 1), 0, wx.EXPAND)
            else:
                self._gripper_sizer_item = sizer.Add(
                    (1, gripper_size), 0, wx.EXPAND)
        else:
        ...
        if self._style & AUI_TB_OVERFLOW:
            ...
            if overflow_size > 0 and self._overflow_visible:
                ...
                if horizontal:
                    self._overflow_sizer_item = sizer.Add(
                        (overflow_size, 1), 0, wx.EXPAND)
                else:
                    self._overflow_sizer_item = sizer.Add(
                        (1, overflow_size), 0, wx.EXPAND)
