...
class FDEMData():
    ...
    def showInfos(self):
        if isinstance(self.x, float):
            ...
            print(len(self.x), "soundings with ", len(self.f), "frequencies")
    ...
    def FOP(self, nlay=2):
        ...
        return pg.FDEM1dModelling(nlay, self.freq(), self.cs, -self.height)
    ...
    def invBlock(self, xpos=0, nlay=2, noise=1.0, stmod=10., lam=100.,
                 lBound=1., uBound=0., verbose=False):
    ...
    def plotData(self, xpos=0, response=None, ax=None, marker='bo-',
                 rmarker='rx-', clf=True, addlabel='', nv=2):
...
#        return FDEM1dModelling(nlay, asvector( self.freq() ), self.cs,
#                               -self.height)
#
# Data file example
